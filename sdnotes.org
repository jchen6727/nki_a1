...
- most in L1/2 (top electrodes) - top-down cortical feedback?? artifact? what if remove top 2 electrodes?
...
in the main code block at the bottom, there are some variables that can be changed (e.g. origDataDir is path to the directory w/ the .mat and .csv layer file, 'test' is an option to either iterate over all of the .mat files in a directory or look at a specific monkey, and dbpath is the path to the .csv file that has the layer info)
